...
class Leitor:
    def __init__(self,master):
        self.master = master
        master.title('Leitor XML v0.7.5')

        # Variáveis auxiliares
        self.ns = {'aux' : 'http://www.portalfiscal.inf.br/nfe'}
        self.tnome = 'NFe_Base'
        self.listad = ['SQL Server Native Client 11.0','SQL Server Native Client 10.0','ODBC Driver 13 for SQL Server','ODBC Driver 17 for SQL Server']
        self.arquivos = 0
        ...
        self.lcaminho.grid(row=0,columnspan = 2)

        self.pathinsert = tk.Entry(master, width=80, borderwidth=2)
        self.pathinsert.grid(row=1,columnspan = 2) 

        self.bpath = tk.Button(master, text='Pesquisar', command=self.get_cam, padx=5)
        self.bpath.grid(row=1,column=2)

        self.lserv = tk.Label(text='Insira o Servidor SQL:')
        self.lserv.grid(row=3,columnspan = 2)

        self.servinsert = tk.Entry(master, width=80, borderwidth=2)
        self.servinsert.grid(row=4,columnspan = 2)
        ...
        self.checkbox2.grid(row=4,column=2)

        self.luser = tk.Label(text='Usuário SQL')
        self.luser.grid(row=5,column=0)

        self.lpswd = tk.Label(text='Senha')
        self.lpswd.grid(row=5,column=1)        

        self.user = tk.Entry(master, width=40, borderwidth=2)
        self.user.grid(row=6)
        self.user.insert(0,os.environ['USERDOMAIN']+'\\'+os.environ['USERNAME']) 
        self.user.configure(state='readonly')

        self.pswd = tk.Entry(master, width=38, borderwidth=2,state='readonly')
        self.pswd.grid(row=6,column=1)

        self.checkbox3= tk.Checkbutton(master, text='Windows Authentication', variable=self.cb3,command=self.auth)
        self.checkbox3.grid(row=6,column=2)        

        self.lbase = tk.Label(text='Insira a base para inserir os dados:')
        self.lbase.grid(row=7,columnspan = 2)

        self.baseinsert = tk.Entry(master, width=80, borderwidth=2)
        self.baseinsert.grid(row=8,columnspan = 2)
        self.baseinsert.insert(0,'DB_XML') 

        self.checkbox1= tk.Checkbutton(master, text='Substituir?', variable=self.cb1)
        self.checkbox1.grid(row=8,column=2)

        self.dpadrao = tk.Label(text='Driver:')
        self.dpadrao.grid(row=9,columnspan = 2)

        self.Lista = ttk.Combobox(master,values=self.listad,textvariable=self.driver)
        self.Lista.config(width=77)
        self.Lista.grid(row=10,columnspan = 2)

        self.barprog = ttk.Progressbar(master,orient='horizontal',length=490,mode='determinate')
        self.barprog.grid(row=12,pady=10,columnspan = 2)

        self.status = tk.Label(textvariable=self.statbar)
        self.status.grid(row=13,columnspan = 2)

        self.bconnect = tk.Button(master, text='Importar XML', command=self.sql_connect)
        self.bconnect.grid(row=15,pady=10,columnspan = 2)
    ...
    def auth(self):
        if self.cb3.get():
            ...
            self.user.configure(state='readonly')
            self.user.update()            
            self.pswd.configure(state='normal')
            self.pswd.delete(0,tk.END)
            self.pswd.configure(state='readonly')
            self.pswd.update()            
        else:
            self.user.configure(state='normal')
            self.user.delete(0,tk.END)
            self.user.update()
            self.pswd.configure(state='normal',show="*")
            self.pswd.delete(0,tk.END)
            self.pswd.update()    
    ...
    def sql_connect(self):
        ...
        try:
            ...
            if self.cb3.get():
                ...
                engine = sqlalchemy.create_engine('mssql://'+self.user.get()+':'+self.pswd.get()+'@'+servidor+'/'+self.baseinsert.get()+'?driver='+sqldriver,fast_executemany=True)
            engine.connect()
            if arquivos == 0:
            ...
            if problema:
                messagebox.showinfo('Erro nos arquivos','Verificar arquivo "Arquivos com problema.txt"!')
            del df_all_xml
            del df_canc_xml
        except Exception as e:
            messagebox.showerror('Erro na leitura, verifique arquivo log.txt',e)
    ...
    def criar_tabela(self):
        if self.cb1.get():
            with engine.connect() as con:
                con.execute('DROP TABLE IF EXISTS [dbo].['+self.tnome+']')
                con.execute('''
                            CREATE TABLE [dbo].['''+self.tnome+'''](
                                ...
                                [nDI] [varchar](20) NULL,
                                [dDI] [date] NULL,
                                [cExportador] [varchar](1000) NULL,
                                [nAdicao] [int] NULL,
                                [nSeqAdic] [varchar](1000) NULL,
                                [cFabricante] [varchar](1000) NULL,
            ...
            with engine.connect() as con2:
                if not engine.dialect.has_table(engine,self.tnome,schema = 'dbo'):
                    con2.execute('''
                                CREATE TABLE [dbo].['''+self.tnome+'''](
                                    [index] [bigint] NOT NULL IDENTITY(1,1) PRIMARY KEY,
                                    [Arq_ID] [bigint] NULL,
    ...
    def log_arqs(self,arquivo,caminho):
        with open(os.path.join(caminho,'Arquivos com problema.txt'), 'a') as f:
            f.write(arquivo+'\n')
    ...
    def ler(self,diretorio,narquivos):
        ...
        for file in os.listdir(diretorio):
            if ((file.endswith(b'.xml') or file.endswith(b'.XML')) and os.stat(os.path.join(diretorio, file)).st_size != 0):
                lastfile = str(file, "utf-8")
                lidos = lidos + 1 
                self.barprog['value']=lidos
                self.barprog.update()
                #print(file)
                try:
                    xroot = et.parse(os.path.join(diretorio, file),parser=et.XMLParser(encoding="utf-8"))
                except:
                    try:
                        xroot = et.parse(os.path.join(diretorio, file),parser=et.XMLParser(encoding="iso-8859-5"))
                    except:
                        self.log_arqs(lastfile,path)
                        continue    
                try:
                    xtree = xroot.getroot()
                    ...
                    if xtree.tag == '{http://www.portalfiscal.inf.br/nfe}nfeProc':
                        ...
                        for itens in xroot.findall('aux:NFe/aux:infNFe/aux:det',self.ns): 
                            ...
                            try:
                                ...
                                dDI = None
                            try:
                                cExportador = itens.find('.*//aux:cExportador',self.ns).text
                            except: 
                                cExportador = None
                            try:
                                nAdicao = itens.find('.*//aux:adi/aux:nAdicao',self.ns).text
                            except: 
                                nAdicao = None
                            try:
                                nSeqAdic = itens.find('.*//aux:adi/aux:nSeqAdic',self.ns).text
                            except: 
                                nSeqAdic = None  
                            try:
                                cFabricante = itens.find('.*//aux:adi/aux:cFabricante',self.ns).text
                            except: 
                                cFabricante = None                                                                                                                                                                                           
                            try:
                            ...
                            leitura = {'Arq_ID':lidos,'Itm_ID':nItem,'chNFe':chNFe,'cUF':cUF,'natOP':natOp,'serie':serie,'nNF':nNF,'dhEmi':dhEmi,'dhSaiEnt':dhSaiEnt,
                                        ...
                                        'uCom':uCom,'vProd':vProd,'vFrete':vFrete,'vSeg':vSeg,'vDesc':vDesc,'vOutro':vOutro,'nDI':nDI,'dDI':dDI,'cExportador':cExportador,
                                        'nAdicao':nAdicao,'nSeqAdic':nSeqAdic,'cFabricante':cFabricante,'xPed':xPed,'orig':orig,'CST_ICMS':CST_ICMS,'vICMS':vICMS,
                                        'vICMSST':vICMSST,'vICMSDeson':vICMSDeson,'CST_IPI':CST_IPI,'vIPI':vIPI,'vBC_II':vBC_II,
                        ...
                        #Pagamentos de Duplicadas e Vencimentos
    ...
    def export(self):
        ...
        for i,call_xml in enumerate(self.chunker(df_all_xml, chunksize)):
            ...
            call_xml.to_sql(name=self.tnome, con=engine, if_exists='append', index=False, schema='dbo',
                        dtype={'chNFe':sqlalchemy.types.VARCHAR(length=44),
                                    ...
                                    'nDI':sqlalchemy.types.VARCHAR(length=20),
                                    'dDI':sqlalchemy.types.Date(),
                                    'cExportador':sqlalchemy.types.VARCHAR(length=1000),
                                    'nAdicao':sqlalchemy.types.INTEGER(),
                                    'nSeqAdic':sqlalchemy.types.INTEGER(),
                                    'cFabricante':sqlalchemy.types.VARCHAR(length=1000),
